...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        Location finalDest = _map.getLabel(segment.get(segment.size() - 1));
        String last = " to " + finalDest.toString();
        Double roadLength = 0.0;
        for (int j = 0; j < segment.size() - 1; j++) {
            if (j < segment.size() - 2) {
                Road road = _map.getLabel(segment.get(j), segment.get(j + 1));
                Road nextRoad = _map.getLabel(segment.get(j + 1),
                        segment.get(j + 2));
                if ((!road.toString().equals(nextRoad.toString()))
                        || (road.direction() != nextRoad.direction())) {
                ...
                Road road = _map.getLabel(segment.get(j), segment.get(j + 1));
                System.out.printf("%d. Take %s %s for %.1f miles%s.",
                        seq, road.toString(), road.direction().fullName(),
                        road.length() + roadLength, last);
                System.out.println();
                seq++;
            }
        }
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road newRoadTo = new Road(name, dir, length);
        Road newRoadBack = new Road(name, dir.reverse(), length);
        _map.add(v0, v1, newRoadTo);
        _map.add(v1, v0, newRoadBack);
    }
